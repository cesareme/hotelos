...
export type GuestIdentityFields = {
  // Salutation / name (Opera Cloud & Mews profile parity).
  title?: string;
  firstName?: string;
  middleName?: string;
  surname1?: string;
  ...
  // Document support number ("número de soporte") required by SES Hospedajes
  // (RD 933/2021) — the issuance/support reference printed on the ID document.
  ...
  // Residence: free-text street address (encrypted at rest) plus the structured
  // locality / province / postal code / country needed for the SES Hospedajes
  // traveller record.
  ...
  email?: string;
  company?: string;
  // Loyalty / VIP / marketing — full guest-profile capture.
  vipCode?: string;
