...
describe("RBAC · SoD · pares estáticos (§4.7)", () => {
  ...
  it("invariantes negativos de mínimo privilegio que las matrices §4.4/§4.5 respetan", () => {
    ...
    // v5 (Tanda RRHH): expediente, convenio y estándares; la plantilla máxima la aprueba dirección general (SoD con payroll.manage).
    // Fusión RRHH sobre main (2026-09-20): users.read se conserva (CIERRE-1) junto a las claves hr.* (la decisión T8a quedó superada por CIERRE-1).
    must("payroll_hr", ["hr.employee.read", "hr.employee.manage", "hr.config.manage", "hr.standards.manage", "compliance.read"]);
    not("payroll_hr", ["payroll.approve", "pms.reservation.create", "guests.manage", "hr.staffing.approve"]);
