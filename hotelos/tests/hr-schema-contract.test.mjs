...
describe("RRHH · migración 20260920173000_rrhh_plantilla_nomina (aditiva, SQL del generador + CHECK a mano)", () => {
  it("está en la cadena tras 20260920160000_checkin_pago_en_recepcion y solo la siguen las dos de L7 (fusión 2026-09-20: ya aplicada con este nombre, no se renumera), con cabecera de la casa", () => {
    // Fusión RRHH sobre main (2026-09-20): la carpeta ya está aplicada en la BD con esta marca; las de la Tanda L7
    // (190000 y 210000) quedan detrás en la cadena. Se fija la cola completa para que el contrato siga siendo estricto.
    assert.deepEqual(migrationFolders.slice(migrationFolders.indexOf(MIGRATION)), [MIGRATION, "20260920190000_portal_huesped_l7", "20260920210000_guest_portal_session_purpose"]);
...
describe("RRHH · RBAC · 5 claves hr.* (recon §3.2)", () => {
  ...
  it("plantillas: payroll_hr (4 hr.* + compliance.read, sin staffing.approve; conserva users.read de CIERRE-1), general_manager (read + staffing.approve), manager / operations_director / owner (read)", () => {
    ...
    assert.equal(templates.payroll_hr.has("users.read"), true, "CIERRE-1 (main, 2026-09-20): payroll_hr + users.read (selector «Persona» → GET /rbac/users), conservada en la fusión de la Tanda RRHH");
    assert.equal(templates.payroll_hr.size, 19, "payroll_hr v3 = 13 keys + users.read (CIERRE-1) + 4 hr.* + compliance.read");
