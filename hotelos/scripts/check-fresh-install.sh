...
# no functions/triggers/views in public → sanity counts → adopt-baseline dry-run
# reports "adopted".
#
...
log "6/8 sin funciones, triggers ni vistas en public (todo el esquema debe venir de migraciones)"
FOREIGN_OBJECTS=$(psql "$TEST_URL" -Atc "
...
[ "$FOREIGN_OBJECTS" = "0" ] || fail "hay $FOREIGN_OBJECTS función/trigger/vista en public que ninguna migración crea"
...
log "OK: $LOCAL_MIGRATIONS migración/es → $TABLES tablas, $ORGS organización/es, $PERMISSIONS permisos, sin drift, sin objetos fuera de Prisma (${elapsed}s)"
if [ "$JSON" = "1" ]; then
