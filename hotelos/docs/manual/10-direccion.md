...
- Recorte al área de contenido (1040×704 píxeles; 1040×480 en la del Centro de informes), sin barra lateral ni cabecera. El aviso «Falta 1 comprobación para poner la propiedad en marcha.» y las tarjetas de instrucciones de cada pantalla están ocultos en las capturas.
...
> En la tabla «Tareas HK» la columna «HABITACIÓN» muestra el número de la habitación y «TAREA» y «ESTADO» van en español («Limpieza de salida», «EN CURSO»); en «Turnos», «ASIGNACIÓN» dice «ASIGNADO» o «SIN ASIGNAR». Las alertas nombran el departamento («RECEPCIÓN», «PISOS», «MANTENIMIENTO»).
...
3. El bloque «Exportar informe» ofrece «Tipo de informe» («Reservas · Facturación · Revenue · Propietario»), «Formato» («PDF · CSV · XLSX · JSON») y el botón «Generar exportación»; «Abrir histórico y previsión» te lleva a **Revenue › Histórico y previsión**.
   ...
   Al pulsar «Generar exportación» el navegador descarga el fichero (`informe-{tipo}-{hotel}-{fecha}.{extensión}`; «XLSX» baja hoy un CSV y «PDF» una página HTML lista para imprimir) y la línea de estado muestra «Exportación lista: informe-….csv». Debajo del formulario aparece el aviso «Exportación lista» con el nombre del fichero, la hora hasta la que sigue disponible y el botón «Descargar exportación», que vuelve a bajar el mismo fichero desde el servidor. Cada exportación se conserva **15 minutos**; pasado ese plazo (o si el servidor se reinicia) el botón responde «Exportación no encontrada o caducada.» y basta con generarla de nuevo. La pestaña «Exportaciones de revenue» (paso 5) y los «Exportar CSV / Exportar Excel» de **Revenue › Histórico y previsión** siguen siendo la vía para los informes del ritual de revenue; el diario y los estados contables tienen sus propias descargas en la guía [20-administracion.md](20-administracion.md).
...
**Resultado esperado.** Tienes descarga inmediata de los informes de revenue (pestaña «Exportaciones de revenue»), la lectura en pantalla de los informes de reservas y facturación, la rentabilidad de 30 días y, si diriges varios hoteles, la cartera consolidada con el detalle de cada uno. Las exportaciones del bloque «Exportar informe» (paso 3) se descargan al momento y quedan 15 minutos en «Descargar exportación».

**Si algo falla.**
- «Descargar exportación» responde «Exportación no encontrada o caducada.»: han pasado más de 15 minutos, el servidor se ha reiniciado o, si el servicio tiene varias réplicas, la petición ha llegado a otra instancia distinta de la que generó el fichero (la exportación vive en la memoria de esa instancia, no en disco ni en almacenamiento compartido); vuelve a pulsar «Generar exportación» y descarga en el momento. «XLSX» sigue bajando un CSV (con extensión `.csv`): ábrelo con Excel o elige «CSV» directamente.
...
> **Nota:** «Comparar plantillas» abre el cajón del mismo nombre: eliges «Plantilla A» y «Plantilla B», lees el resumen («N claves solo en A», «N claves comunes», «N claves solo en B») y la tabla «Claves por módulo del diccionario §4.3», que es la tabla de accesos por departamento (qué puede hacer cada plantilla en cada módulo; paso a paso en [60-sistemas.md](60-sistemas.md), apartado 1.5). El interruptor «Exigir doble factor (2FA)» deja solo la marca «2FA: Activo» en la ficha: la marca no es un control de acceso, la aplicación no pide un segundo factor al entrar (decisión documentada en la guía de sistemas, apartado 1.2).
...
**Resultado esperado.** Sabes qué falta para que el hotel salga en vivo y quién debe hacerlo. Estos bloqueos son los que producen el aviso «Falta 1 comprobación para poner la propiedad en marcha.» que ves en la cabecera de todas las pantallas; «Ahora no» lo oculta solo durante la sesión.
...
| «Algo ha fallado en la interfaz · El error ya ha sido reportado al equipo. Puedes intentarlo de nuevo.» | Error de pantalla | «Reintentar»; si se repite, anota la pantalla y avisa a sistemas |
...
| «Falta 1 comprobación para poner la propiedad en marcha.» | La lista de «Salida en vivo» tiene un bloqueante (en la demo, SES.Hospedajes en modo de pruebas) | «Ahora no» para ocultarlo en esta sesión; resuélvelo desde Puesta en marcha › «Salida en vivo» |
...
- **Textos en inglés.** Varias etiquetas de Mi día de dirección, del catálogo del Centro de informes y de las pantallas de configuración de la Puesta en marcha siguen en inglés.
- **«Descartar» en anomalías** no funciona hoy (ver tarea 1).
- **«Generar exportación» del Centro de informes** descarga el fichero y lo deja disponible 15 minutos en «Descargar exportación» (tarea 7, corregida); el enlace solo vale en la misma instancia del servidor que lo generó (memoria, no almacenamiento compartido) y «XLSX» baja hoy un CSV. Las descargas de «Exportaciones de revenue» y de Histórico y previsión no cambian.
- **Asistente ehotelOS.** Sin modelo de lenguaje responde por reglas: cada pregunta sugerida se enruta a una herramienta que consulta datos reales y la respuesta cita la fuente. «¿Cuál es la ocupación ahora mismo?» contesta «Ocupación hoy: 11,1 % (2/18 habitaciones)» (fuente `get_occupancy_today`) y «¿Cuántas llegadas tengo hoy?» contesta «N reservas con llegada hoy.» (fuente `get_arrivals_today`; «4 reservas con llegada hoy.» en la demo). El enrutado reconoce las palabras clave en cualquier orden y sin signos ni acentos («llegadas», «salidas», «ocupación», «pickup», «saldo pendiente», «pisos», «cumplimiento»…); una pregunta sin ninguna de ellas («hola») devuelve «No he sabido enrutar tu pregunta a una herramienta concreta…» con el catálogo de herramientas.
