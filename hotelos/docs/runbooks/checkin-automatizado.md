...
| Derivación al mostrador (corrector L7-REV-05) | `POST …/handoff` `{ kind: "signature" }` (con `x-kiosk-token` el kiosco queda como origen) | `GuestHandoffResult { sessionId, reservationId, status: "handed_off", handoffKind: "signature_pending", ticket: "K-nnnn", kioskDeviceId, idempotent }`: «Firmar en recepción» pasa la sesión a `handed_off` con `handoffReason «Firma en recepción · ticket K-nnnn»`, auditoría `CheckInHandedOff` y kind `signature_pending` en la cola de Mi día; segunda llamada → mismo ticket, `idempotent: true`; `409 CHECKIN_SESSION_CLOSED` con la sesión `checked_in` \| `expired` \| `cancelled`. Recepción cierra con `POST /reservations/:id/check-in/resolve-handoff`. |
