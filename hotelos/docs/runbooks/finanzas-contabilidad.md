...
| `VatBookSourceType` | `invoice` · `rectification` · `simplified` · `supplier_bill` · `expense` · `sage200` | Documento origen de una fila del libro. `sage200` (Tanda 7c, §19): fila importada del Libro Registro de IVA de Sage 200 (`sourceId <empresa>:<ejercicio factura>:<serie>:<factura>[:<NIF>][:<recepción>][:R]`; el NIF y la recepción —nº de recepción del libro o, si falta, la fecha ISO de expedición; FIX-1 · F4— solo en recibidas); `rebuildVatBooks` la conserva y el lote omite los documentos propios de ehotelOS (nunca la misma factura como `sage200` e `invoice` / `supplier_bill`). Primer `ALTER TYPE … ADD VALUE` de la cadena de migraciones (`20260917110000_sage200_importacion`). |
...
| | `POST /treasury/bank-accounts/:id/statements/import`, `POST|DELETE /treasury/bank-lines/:bankLineId/reconcile`, `POST /treasury/statements/:id/auto-reconcile`, `POST /treasury/sepa/remittances` (toda remesa persistida audita `SEPA_REMITTANCE_GENERATED`, corrector CIERRE-1 · REV-01; solo adeudos Norma 19: `kind: norma34` → 403 `SUPPLIER_PAYMENT_ROUTE_REQUIRED` antes de parsear y sin fila, corrector CIERRE-1 · REV-02 — las transferencias a proveedores salen únicamente por `supplier-payments`), `POST …/remittances/:id/status` | `banking.reconcile` (servicio: o `accounting.journal.post`) | high |
| | `POST /treasury/sepa/supplier-payments` (remesa Norma 34 de facturas de proveedor; `treasury/route-permissions.partial.ts:41`) | `payables.pay` + separación de funciones dinámica por factura (`assertSupplierBillPaymentAuthorized`, Tanda CIERRE-1): registrador ≠ pagador y aprobador ≠ pagador salvo `controller` en el centro; 409 `RBAC_SOD_CONFLICT` (`details.rule` `creator_ne_payer` / `approver_ne_payer`, `authorUserId`, `billId`) rechaza la remesa COMPLETA antes de construir el Norma 34 (fail-closed: ninguna factura entra en `skipped`, nada se persiste con o sin `generate`); platform admin y break-glass pasan como `privileged`; orden por factura: existencia → `posted` → no pagada → SoD → IBAN del proveedor; la respuesta lleva `billIds` + `sod` por factura y, con `generate: true`, la remesa persistida guarda `billIds` + `sod` en `payloadJson`, los expone en `GET /treasury/sepa/remittances[/:id]` (`banking.read`; corrector CIERRE-1 · FUN-07) y audita `SEPA_REMITTANCE_GENERATED` con ellos (corrector CIERRE-1 · REV-01; `auditoria-eventos.md` §6.3); el cuerpo devuelto sin `generate` NO se puede reenviar a `POST /treasury/sepa/remittances` (403, REV-02) | high |
...
| | `GET /accounting/ledger-imports/third-parties` (`?q=&role=&limit=&cursor=`; directorio de terceros importados, FIX-1 · F11; `name` solo de sociedades acreditadas por NIF) | `accounting.reports.read` efectiva · R11 (Tanda CIERRE-1): `assertFinanceReadScope(context, null)` como libros / 347 / diario → sin `accounting.entity.read` ni ámbito de toda la sociedad, 404 `ENTITY_SCOPE_REQUIRED` (`requiredPermission = accounting.entity.read`) con mensaje propio del directorio (corrector CIERRE-1 · FUN-01: «…es de toda la sociedad y no tiene dimensión de centro; requiere el permiso accounting.entity.read», nunca «indica el centro (propertyId)» porque la consulta `.strict()` responde 400 a `propertyId`; la pantalla «Terceros» muestra un estado propio a los perfiles de centro sin lanzar la petición); un usuario de ámbito de centro (manager / admin_clerk con `accounting.reports.read` + `accounting.read`) no lista los terceros de la sociedad | medium |
