...
| Calendario / parrilla | 5 | `operations/ShiftManagerScreen.tsx` (cronología) · `revenue/RateGridEditorScreen.tsx` (parrilla, ola 5) · `timeline/LiveTimelineWorkspace.tsx` (cronograma, ola 3) · `operations/GroupsCalendarScreen.tsx` (Gantt sin `<table>`, ola 3) | `Calendario` | 3.7 (`CocoaScrollArea`) · 3.1 (`fullBleed`) | `CocoaPage fullBleed density="compact"` → `CocoaScrollArea axis="x" stickyFirstColumn` con `<table data-cocoa-grid-table>` (única `<table>` cruda permitida) → `CocoaActionBar` de publicación · cabecera de fechas sticky **sin blur** | 40 | regla 3 (`data-cocoa-grid-table` dentro de `CocoaScrollArea`), D24 (`fullBleed` solo en el cuerpo), prohibición de blur (§6) |
...
| 11 | Compartido, desarrollo y limpieza | 10 | 5841 | **129** | 6 · 1 · 2 · 1 | ≈ 12–15 días·persona | 1 | 3 | 2 |
| **Total** | | **10** | **5841** | **129** | 6 · 1 · 2 · 1 | ≈ 12–15 días·persona | 1 | 3 | 2 |
...
**0 ficheros** · 0 líneas · **0 puntos** · S 0 · M 0 · L 0 · XL 0 · ≈ 0 días·persona · ya migradas: `aiOperations/AiHumanReviewQueueScreen.tsx`, `aiOperations/AiOwnerSummaryScreen.tsx`, `assistant/AssistantChatScreen.tsx`, `operations/FrontDeskActionQueue.tsx`, `operations/FrontDeskDashboard.tsx`, `operations/GeneralManagerScreen.tsx`, `operations/NightAuditScreen.tsx`, `operations/OperationsDirectorScreen.tsx`, `operations/QuickCheckInDrawer.tsx`, `operations/QuickCheckOutDrawer.tsx`, `operations/ShiftManagerScreen.tsx`, `owner/OwnerHomeScreen.tsx`, `tabs/hoy/MiDiaTabs.tsx`

### Ola 3 · Recepción

**0 ficheros** · 0 líneas · **0 puntos** · S 0 · M 0 · L 0 · XL 0 · ≈ 0 días·persona · ya migradas: `admin/AllotmentsScreen.tsx`, `guestJourney/GuestJourneyWorkspace.tsx`, `guests/GuestProfileScreen.tsx`, `guests/GuestsListScreen.tsx`, `guests/GuestTimelineScreen.tsx`, `operations/ConciergeInboxDashboard.tsx`, `operations/GroupDetailDialog.tsx`, `operations/GroupsCalendarScreen.tsx`, `operations/GroupsEventsDashboard.tsx`, `operations/GroupsPickupCard.tsx`, `operations/NewEventDialog.tsx`, `operations/NewGroupDialog.tsx`, `operations/RoomBlockGridDialog.tsx`, `operations/RoomingListImportDialog.tsx`, `operations/RoomRackScreen.tsx`, `reservations/ReservationAgentScreen.tsx`, `reservations/ReservationCreateScreen.tsx`, `reservations/ReservationImportScreen.tsx`, `reservations/ReservationsListScreen.tsx`, `reservations/ReservationWorkspaceScreen.tsx`, `tabs/recepcion/GruposEventosTabs.tsx`, `tabs/recepcion/HuespedesTabs.tsx`, `tabs/recepcion/NuevaReservaTabs.tsx`, `tabs/recepcion/ReservasTabs.tsx`, `timeline/LiveTimelineWorkspace.tsx`

### Ola 4 · Operaciones

**0 ficheros** · 0 líneas · **0 puntos** · S 0 · M 0 · L 0 · XL 0 · ≈ 0 días·persona · ya migradas: `admin/FnbInventoryScreen.tsx`, `admin/FnbMenuScreen.tsx`, `operations/AssetsDashboard.tsx`, `operations/EnergyDashboard.tsx`, `operations/HousekeepingDashboard.tsx`, `operations/HousekeepingMobileScreen.tsx`, `operations/InventoryDashboard.tsx`, `operations/MaintenanceDashboard.tsx`, `operations/MaintenanceMobileScreen.tsx`, `operations/PosDashboard.tsx`, `operations/ProcurementDashboard.tsx`, `operations/SafetyDashboard.tsx`, `operations/WorkforceDashboard.tsx`, `pos/CashClosureScreen.tsx`, `tabs/operaciones/ComprasInventarioTabs.tsx`, `tabs/operaciones/MantenimientoTabs.tsx`, `tabs/operaciones/PisosTabs.tsx`, `tabs/operaciones/PuntoVentaTabs.tsx`
...
| 2 | `ModuleSettingsPlaceholder.tsx` | — · fábrica | — | formulario · Formulario | S · 179 · **16** | card 3 · bo 10 · btn 3 · st 18 · h1 1 | 15 | `makeModulePlaceholder` (App.tsx; 16 placeholders, presupuesto 20 de check-discoverability) |
| 3 | `dev/StyleGuideScreen.tsx` | `StyleGuideScreen` · dev-only · alojada en AuditLogViewer | `/desarrollo/guia-estilo` | asistente · Asistente | XL · 2538 · **10** | st 40 | 25 (defecto) | ya en `CocoaPage` (37 `style={` de muestras): `STYLE_BUDGET` 40 y salir de `NOT_MIGRATED` en cuanto el integrador lo decida (puede ir con la ola 1) |
| 4 | `ScreenScaffold.tsx` | — · sub-vista | — | otro · PlantillaBase | S · 132 · **9** | card 4 · bo 6 · btn 1 · st 2 | 25 (defecto) | solo lo usa OnboardingScreens |
...
| 8 | `tabs/tab-helpers.tsx` | — · contenedor de pestañas | — | contenedor · — (NavItemTabs) | S · 165 · **1** | st 4 · h1 1 | — | pinta el `<h1>` alojado (`HostedHead`): exención de la regla 8 o traslado a `components/` antes de salir de `NOT_MIGRATED` |
| 9 | `tabs/NavItemTabs.tsx` | — · contenedor de pestañas | — | contenedor · — (NavItemTabs) | S · 123 · **0** | st 1 | — | ya Cocoa (`CocoaPageHeader` + `CocoaRouteTabs`); solo salir de `NOT_MIGRATED` |
| 10 | `tabs/TabHost.tsx` | — · contenedor de pestañas | — | contenedor · — (NavItemTabs) | S · 91 · **0** | h1 1 | — | pinta el `<h1>` alojado: misma exención de la regla 8 que `tab-helpers.tsx` |
<!-- cocoa-22-waves:end -->
...
| R14 | `styles/mobile.css:257-260` | el alta de canal ya es `CocoaFormRow` + `CocoaSelect` (ola 7); el tope genérico de `<select>` se retira cuando `grep -rn '<select' apps/admin-web/src/screens` devuelva 0 (olas 10 · 11) | ola 11 |
...
| R31 | `components/cocoa-rate-grid/helpers.ts:956-969` · `__tests__/final-fixes.test.mts:137-140` | `toastOffsetForBar` / `STATUS_BAR_BASE_HEIGHT_PX` / `TOAST_OFFSET_BASE_PX` ya no tienen consumidor (el offset es el contrato de `CocoaActionBar`): retirar helper y test juntos | ola 11 (limpieza) |
| R32 | `packages/ui/src/components/timeline/**` (14 ficheros) · `apps/admin-web/vite.config.ts:18,38` (alias `@hotelos/ui/timeline`) | sin ningún consumidor en admin-web desde la ola 3: retirar el kit y el alias, o migrarlo a tokens si otro paquete lo usa | ola 11 |
