...
Tanda L6b (2026-09-20): el asistente unificado consume tool use — `modules/assistant/assistant-core.service.ts` (`answerByModel`) pasa `tools` (catálogo filtrado por RBAC × superficie × módulos más las escrituras del registro como propuesta) y `toolChoice: "auto"`, con `toolResultBlock` en un bucle de ≤ 3 turnos y respaldo por reglas con aviso; runbook [`docs/runbooks/asistente-ia.md`](../runbooks/asistente-ia.md) §4. `insight` sigue sin consumidor.
...
| conversaciones del asistente unificado (Tanda L6b) | `assistant_conversations` (una por usuario + propiedad + superficie: `surface`, `title` cifrado por `PII_FIELDS.AssistantConversation` (corrector REV-02: el redactor no reconoce nombres sin tratamiento), `screen_context_json` solo con ids seguros (REV-03), `status`, `last_message_at`; purga por retención `[assistant.purge]` 90 días y supresión RGPD de los hilos `guest:<id>`, L6B-REV-07) y `assistant_messages` (FK en cascada; `content` cifrado por `PII_FIELDS.AssistantMessage` y redactado de forma irreversible antes de persistir, `tool_calls_json` con las citas, `routed_by`, `model`, `tokens_*`, `cost_eur`); una fila `answerAnalyticsQuestion` por turno en `ai_tool_calls` cuyo `conversation_id` es `assistant_conversations.id` (sin FK; también en las llamadas del runner lanzadas desde el asistente, `source: "chat"`): `RunnerContext.conversationId` deja de significar la conversación de mensajería del huésped |
...
8. **Redacción por regex**: cubre nombres con tratamiento, con fórmula de presentación/despedida y los sembrados por `knownPii`; **no** un nombre suelto sin ninguna de esas pistas ni direcciones; los bytes de documentos viajan íntegros → DPA/ZDR antes de procesar documentos reales. Consecuencia en el asistente unificado (corrector L6b · REV-02): lo que guarda texto libre del usuario va cifrado (`content`, `title`) y la telemetría del turno guarda solo la huella de la pregunta (`questionChars` + `questionSha256`), nunca el texto.
...
11. **L6b — hecho (2026-09-20)**: asistente unificado (chat del back office + copiloto de recepción + bot del huésped sobre un solo núcleo) con memoria y `tools` del núcleo, panel en el shell y front honesto (`ai-operations-labels.ts` con `skipped`, «Encendida · Sin modelo / En uso» solo por llamadas reales): runbook [`docs/runbooks/asistente-ia.md`](../runbooks/asistente-ia.md) e informe `docs/audits/TANDA-L6B-ASISTENTE-2026-09-20.md`. Quedan fuera de L6b: briefing diario (`insight` con Opus 5 sigue sin consumidor), respuesta a reseñas desde el panel (`draftReviewResponse` solo como lectura del catálogo), móvil (`services/api.ts` sin `Authorization`) y el humo con clave real (tool use verificado solo con `fetch` simulado).
...
| 5 | **Autonomía de escrituras** | `WRITE_ALWAYS_CONFIRMS = true`: ninguna de las escrituras se ejecuta sin persona, ni con nivel `autonomous` (que además exige `configurationJson.autonomousApprovedBy`). **Decisión aplicada en L6b (2026-09-20, D4 del informe):** sin autonomía; el asistente unificado ofrece al modelo las 13 escrituras con implementación (módulo activo y permisos del usuario, ninguna en la superficie del huésped) SOLO como propuesta `awaiting_confirmation` que una persona decide en el panel o en Pendientes IA (runbook `docs/runbooks/asistente-ia.md` §7) | Si en una tanda posterior alguna escritura de bajo riesgo (`createHousekeepingTask`, `markRoomClean`) puede ser autónoma con `automationLevel = autonomous` y aprobador registrado |
...
| 16 | **Pendientes IA y ejecución** | **Decidido y aplicado en L6b (L6b-09, 2026-09-20):** la pantalla de Pendientes IA llama a `POST /ai/tool-calls/:id/confirm` al aprobar o rechazar un ítem `ai_tool_call` («Aprobar y ejecutar» / «Rechazar»; el runner ejecuta o cierra la fila y cierra la revisión; resultados Ejecutada · Ejecución fallida · Rechazada · Caducada 409 · Sin permiso 403 nombrando las claves); el `approve` de la cola ya no se usa para esos ítems y el panel del asistente decide sus propias propuestas por la misma ruta (runbook §7) | Si además debe invocarse `/ai-operations/review/:id/approve` (habría que tolerar el 409 «ya decidida» posterior) y qué hacer con los ítems aprobados en la cola ANTES de la tanda (fila `awaiting_confirmation` con revisión `approved`: botones deshabilitados, solo confirm directo) |
