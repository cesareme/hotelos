...
export function aeatVatBookXlsx(options: { expedidas?: readonly VatFixtureRow[]; recibidas?: readonly VatFixtureRow[] } = {}): Buffer {
  ...
  const recibidasRows: XlsxCell[][] = recibidas.map((row) => ["2026", "3T", "01", "A", "681", row.tipoFactura ?? "F1", "G01", "S", row.fecha, "", row.fecha, row.serie, row.numero, "", "", row.nif, row.nombre, "ES", "01", "S1", "", row.total, row.base, row.tipo, row.cuota, row.cuota, "", "", "N", "", "", "", "", "", row.retencion ?? "", ""]);
  return writeXlsx(
    [
