...
describe("role templates", () => {
  ...
  it("owner is a strict subset of the org scope, break_glass is the whole org scope, admin holds no money key of the SoD pairs (Tanda 8a)", () => {
    ...
    assert.equal(ROLE_TEMPLATE_VERSION, 5); // v5 (Tanda RRHH · hr.*): aditiva, ver packages/shared/src/permissions.ts
  });
...
/** The org scope of template version 1 (the HEAD before Tanda 8a): today's catalog minus the 27 keys L0 added and the 4 documents.* keys Tanda T9 added (v4) and the 5 hr.* keys Tanda RRHH added (v5). */
const V1_NEW_KEYS = new Set<string>([
  ...
  // Tanda RRHH (template v5): the five hr.* keys did not exist in v1 either.
  "hr.employee.read", "hr.employee.manage", "hr.config.manage", "hr.standards.manage", "hr.staffing.approve"
]);
...
describe("upgradeRoleTemplate (fake store)", () => {
  ...
  it("backfillTemplateRoles({ upgrade: true }) upgrades managed template roles (dry-run reports, real run writes) and skips managed = false", async () => {
    ...
    assert.ok(plan.upgraded?.some((line) => line.startsWith("Dirección (org_a) ← manager v0→v5: +") && line.endsWith("−16 [dry-run]")));
    assert.ok(plan.upgraded?.some((line) => line.startsWith("Owner (org_a) ← owner v0→v5:")));
