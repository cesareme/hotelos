...
// checks and connector health notes used to arrive from the API with
// environment variable names («SES_HOSPEDAJES_MODE=sandbox»), English field
// keys («postalCode»), region codes («ES_CANARIAS») and engineering words
// («sandbox», «stub», «flag»), and this module rewrote them on the way to the
// screen. Since ola 11 the API humanises them at the source (backoffice.service.ts
// computeReadiness, compliance-health.service.ts, packages/compliance
// verifactu/software.ts; unit-tested there), so the rewrite table is empty and
// readinessMessage() is only the normaliser every screen keeps calling: an
// unknown or future template comes back unchanged, null / undefined read as "".
// A new fragment that still needs a client-side rewrite goes here (with its
// test in lib/__tests__/readiness-message.test.mts) until the API is fixed.

type MessageReplacer = (match: string, ...groups: string[]) => string;
type MessageRewrite = [RegExp, string | MessageReplacer];

// Order matters when non-empty: whole sentences first, then fragments, then the loose words.
const READINESS_MESSAGE_REWRITES: MessageRewrite[] = [];

/**
 * readinessMessage("SES.HOSPEDAJES en modo de pruebas: los partes van a un simulador, …") → unchanged
 * readinessMessage("  3 tipo(s) de habitación activos. ") → "3 tipo(s) de habitación activos." · null → "".
 */
