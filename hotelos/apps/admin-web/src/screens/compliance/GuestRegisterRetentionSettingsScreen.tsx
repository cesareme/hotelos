...
// the real values and says so. The frame is CocoaPage on the host context
// (hosted, the container paints eyebrow and H1; states, skeleton and ⌘K commands
// are the page's); the body is Cocoa 22: a callout, two sections of key/value
// rows and the note on sensitive data.
...
import { CocoaBadge, CocoaButton, CocoaCallout, CocoaGrid, CocoaPage, CocoaSection, CocoaSkeleton, CocoaSpan } from "../../components/cocoa";
...
export function GuestRegisterRetentionSettingsScreen() {
  ...
  return (
    <CocoaPage
      ...
      skeleton={<CocoaSkeleton.Grid rows={[[12], [6, 6], [12]]} height={160} label="Cargando la política de retención" />}
      error={{ title: "No se pudo cargar la política", message: error ?? undefined, onRetry: () => void load() }}
      commands={[{ id: "conservacion-actualizar", label: "Actualizar la política de retención", run: () => void load() }]}
    >
