...
const CATEGORIES = ["Standard", "Superior", "Suite", "Apartment", "Dorm"];
const BED_TYPES = ["King", "Queen", "Twin", "Sofa bed"];

export function RoomTypeFormScreen(props: { onNavigate?: (route: string) => void }) {
  ...
  const issues: string[] = [];
  if (!values.name.trim()) issues.push("Room type name is required.");
  if (!values.code.trim()) issues.push("Room type code is required.");
  if (!values.category) issues.push("Category is required.");
  if (!values.baseOccupancy) issues.push("Base occupancy is required.");
  if (!values.maxOccupancy) issues.push("Max occupancy is required.");

  function handleSave(addAnother: boolean) {
    if (issues.length > 0) {
      console.log("[mobile/RoomTypeForm] validation issues", issues);
      return;
    }
    console.log("[mobile/RoomTypeForm] save", { ...values, addAnother });
    if (addAnother) {
      setValues({ name: "", code: "", category: "", baseOccupancy: "", maxOccupancy: "", maxAdults: "", maxChildren: "", defaultBedType: "", sellable: true, baseRate: "" });
    }
  }
  ...
  return (
    <View style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.kicker}>Configuration / Room types</Text>
        <Text style={styles.title}>Room type</Text>
        <Text style={styles.body}>Defaults, features, occupancy and base rate for a room type.</Text>
        <View style={styles.chips}>
          <StatusChip label="room_types.manage" tone="info" />
        ...
        <Section title="Identity">
          <Field label="Name" required>
            <TextInput style={styles.input} value={values.name} onChangeText={(value) => patch("name", value)} placeholder="Junior Suite" placeholderTextColor={colors.muted} />
          </Field>
          <Field label="Code" required>
            <TextInput style={styles.input} value={values.code} onChangeText={(value) => patch("code", value)} placeholder="JSU" placeholderTextColor={colors.muted} />
          </Field>
        ...
        <Section title="Occupancy">
          <Field label="Base occupancy" required>
            <TextInput style={styles.input} value={values.baseOccupancy} onChangeText={(value) => patch("baseOccupancy", value)} keyboardType="number-pad" placeholderTextColor={colors.muted} />
          </Field>
          <Field label="Max occupancy" required>
            <TextInput style={styles.input} value={values.maxOccupancy} onChangeText={(value) => patch("maxOccupancy", value)} keyboardType="number-pad" placeholderTextColor={colors.muted} />
          </Field>
          <Field label="Max adults">
            <TextInput style={styles.input} value={values.maxAdults} onChangeText={(value) => patch("maxAdults", value)} keyboardType="number-pad" placeholderTextColor={colors.muted} />
          </Field>
          <Field label="Max children">
            <TextInput style={styles.input} value={values.maxChildren} onChangeText={(value) => patch("maxChildren", value)} keyboardType="number-pad" placeholderTextColor={colors.muted} />
          </Field>
          <Field label="Default bed type">
            <OptionRow options={BED_TYPES} value={values.defaultBedType} onChange={(value) => patch("defaultBedType", value)} />
          </Field>
        </Section>

        <Section title="Pricing and state">
          <Field label="Base rate">
            <TextInput style={styles.input} value={values.baseRate} onChangeText={(value) => patch("baseRate", value)} keyboardType="decimal-pad" placeholder="0.00" placeholderTextColor={colors.muted} />
          </Field>
      ...
      <View style={styles.actionBar}>
        ...
        <Pressable style={styles.danger} onPress={() => console.log("[mobile/RoomTypeForm] deactivate")}>
